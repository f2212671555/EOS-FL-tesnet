...
class [[eosio::contract]] model : public contract {
  ...
  public:
    ...
    [[eosio::action]]
    void enroll(name owner, uint64_t num,std::string hash){
      require_auth(owner);
    ...
    [[eosio::action]]
    void record(name owner, uint64_t id, uint64_t epochid, uint64_t turn, std::string time, std::string accuracy){
      require_auth(owner);
  ...
  private:
    ...
    struct [[eosio::table]] task {
      ...
      std::string status; // task status "WAITING" "AGGREGATING"
